...
//  2021-XX-XX: DirectX11: Add support for ImGuiBackendFlags_RendererHasTexReload.
...
static void ImGui_ImplDX11_UpdateFontsTexture()
{
    ...
    if ((!bd->pFontTextureView) || (bd->FontTextureWidth != width) || (bd->FontTextureHeight != height))
    {
        ...
        // Release old texture
        if (bd->pFontTextureView) { bd->pFontTextureView->Release(); bd->pFontTextureView = NULL; ImGui::GetIO().Fonts->SetTexID(NULL); } // We copied g_pFontTextureView to io.Fonts->TexID so let's clear that as well.
        if (bd->pFontTexture) { bd->pFontTexture->Release(); bd->pFontTexture = NULL; }
        ...
        // Create new texture
        {
            ...
            IM_ASSERT(bd->pFontTexture != NULL);
...
void    ImGui_ImplDX11_InvalidateDeviceObjects()
{
    ...
    if (bd->pFontTexture)           { bd->pFontTexture->Release(); }
    if (bd->pIB)                    { bd->pIB->Release(); bd->pIB = NULL; }
    if (bd->pVB)                    { bd->pVB->Release(); bd->pVB = NULL; }
